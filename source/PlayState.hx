...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch(songLowercase)
		{
			...
			case 'senpai' | 'roses' | 'thorns' | 'calm' | 'dominating' | 'skills' | 'envy':
		...
		switch (SONG.player2)
		{
			...
			case 'kite':
				...
				if (FlxG.save.data.distractions)
					{
						...
						if (!PlayStateChangeables.Optimize)
						{
							kitetrail = new FlxTrail(dad, null, 4, 24, 0.3, 0.069);
							kitetrail.color = 0xFFFFFFFF;
							// evilTrail.changeValuesEnabled(false, false, false, false);
							// evilTrail.changeGraphic()
						}
						// evilTrail.scrollFactor.set(1.1, 1.1);
				...
				dad.y += 170;
				//var kitetrail = new FlxTrail(dad, null, 4, 24, 0.3, 0.069);
			    //kitetrail.color = 0xFFFFFFFF;
		}
		...
		if (isStoryMode)
		{
			switch (StringTools.replace(curSong, " ", "-").toLowerCase())
			{
				...
				case 'thorns':
					schoolIntro(doof);
				case 'calm':
					var introText:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('warning'));
	...
	override function stepHit()
	{
		...
		if (dad.curCharacter == 'kite' && SONG.song.toLowerCase() == 'dominating')
		{
			if (curStep == 685)
				{
					FlxTween.tween(FlxG.camera, {zoom: 1.5}, 2, {
		...
		if (dad.curCharacter == 'kite' && SONG.song.toLowerCase() == 'skills')
		{
			//712
			//1306 1362
			if (curStep == 1306)
		...
		if (dad.curCharacter == 'kite' && SONG.song.toLowerCase() == 'envy')
		{
			if (curStep == 1273)
				{
					FlxG.camera.fade(FlxColor.WHITE, .7, true);
